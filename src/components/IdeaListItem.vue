<template>
  <q-item>
    <q-item-main>
      ...
      <span v-if="showBible">
        <q-chip v-for="bibleRef in idea.bibleRefs" small color="secondary" :key="bibleRef._id">{{ $bible.readable(bibleRef) }}</q-chip>
      </span>
    </q-item-main>
...
export default {
  ...
  data () {
    return {
      ...
      editOpen: false,
      idea: this.ideaObj,
      addState: this.$selectedTopic.get() && !this.$selectedTopic.find(this.ideaObj._id) ? 'n' : 'y'
    }
  },
  watch: {
    idea (value) {
